...
namespace CommonAPUtil.APInventoryUtil
{
    ...
    /// 在庫更新：売上画面用
    /// </summary>
    public class SalesReturnInventory
    {
        /// <summary>
        /// ログ出力用インスタンス
        /// </summary>
            ...
            List<InvDao.InventoryParameter> paramListInOutParam, string paramLanguage)
        {
            ...
            //"売上：在庫更新処理"
            returnMessage = ComST.GetPropertiesMessage(new string[] { ComID.MB10083, ComID.MB10089 }, paramLanguage);
            ...
            foreach (var paramInfo in paramListInOutParam)
            {
                ...
                // ---　 受払履歴更新・ロット在庫更新　----
                if (paramInfo.InoutDivision == ComConst.INOUT_DIVISION.SALES_ACCEPTANCE_CRITERIA_REGIST)
                {
                    // 　【検収基準】売上出荷　(処理区分=0:通常データ)
                    retVal = saleShipmentRegist(db, paramInfo, inIdx, strLanguage, ref returnMessage);
                    if (!retVal)
                    {
                        break;
                    }
                ...
                else if (paramInfo.InoutDivision == ComConst.INOUT_DIVISION.SALES_ACCEPTANCE_CRITERIA_CANCEL)
                {
                    // 　【検収基準】売上出荷取消　(処理区分=9:取消データ)
                    retVal = saleShipmentCancel(db, paramInfo, inIdx, strLanguage, ref returnMessage);
                    if (!retVal)
                    {
                        break;
                    }
                ...
                else if (paramInfo.InoutDivision == ComConst.INOUT_DIVISION.SALES_RETURN_REGIST)
                {
                ...
                else if (paramInfo.InoutDivision == ComConst.INOUT_DIVISION.SALES_RETURN_CANCEL)
                {
            ...
            string paramLanguageId, ref string paramMessage)
        {
            ...
            // 受払区分(値チェック)
            if (!((paramInfo.InoutDivision == ComConst.INOUT_DIVISION.SALES_ACCEPTANCE_CRITERIA_REGIST)
                || (paramInfo.InoutDivision == ComConst.INOUT_DIVISION.SALES_ACCEPTANCE_CRITERIA_CANCEL)
                || (paramInfo.InoutDivision == ComConst.INOUT_DIVISION.SALES_RETURN_REGIST)
                || (paramInfo.InoutDivision == ComConst.INOUT_DIVISION.SALES_RETURN_CANCEL)))
            {
                //@行目の受払区分が正しくありません。
        ...
        /// <param name="paramMessage">正常時:空白 エラー時：該当メッセージ</param>
        /// <returns>true:正常 false：エラー</returns>
        private static bool saleShipmentRegist(ComDB db, InvDao.InventoryParameter paramInfo, int paramIdx,
            string paramLanguageId, ref string paramMessage)
        {
            bool retVal = true;
        ...
        /// <param name="paramMessage">正常時:空白 エラー時：該当メッセージ</param>
        /// <returns>true:正常 false：エラー</returns>
        private static bool saleShipmentCancel(ComDB db, InvDao.InventoryParameter paramInfo, int paramIdx,
            string paramLanguageId, ref string paramMessage)
        {
            bool retVal = true;
