...
namespace CommonTMQUtil
{
    ...
    public static partial class CommonTMQUtil
    {
        ...
        public static class StructureLayerInfo
        {
            ...
            /// <summary>
            /// データクラスに予備品の場所階層情報を設定する処理
            /// </summary>
            /// <typeparam name="T"></typeparam>
            /// <param name="targetList"></param>
            /// <param name="db"></param>
            /// <param name="languageId"></param>
            /// <param name="treeViewFlg"></param>
            [Obsolete("階層の仕様変更により、SetStructureLayerInfoToDataClassを予備品階層で使用して問題ありません。このメソッドは削除予定です。")]
            public static void SetSpareInfoToDataClass<T>(ref IList<T> targetList, ComDB db, string languageId, bool treeViewFlg = false)
            {
                // 予備品の階層情報を取得(倉庫、棚)
                TMQUtil.StructureLayerInfo.SetStructureLayerInfoToDataClass<T>(ref targetList, new List<StructureType> { StructureType.SpareLocation }, db, languageId, treeViewFlg);
            }
        ...
        public class ScheduleListConverter
        {
            ...
            /// <returns></returns>
            private bool isEqualDateDisplayUnit(ScheduleDisplayUnit unit, DateTime target, DateTime keyDate, int monthStartNendo)
            {
                // 比較用の値を単位に応じて集計
                DateTime compareTarget = convertDateByUnit(unit, target, monthStartNendo);
                // 比較
            ...
            private DateTime convertDateByUnit(ScheduleDisplayUnit unit, DateTime target, int monthStartNendo)
            {
            ...
            protected virtual List<Dao.ScheduleList.Get> getGroupDataList(IList<Dao.ScheduleList.Get> dataList, string keyId, long? groupKey, ScheduleDisplayUnit unit, DateTime keyDate, int monthStartNendo)
            {
                return dataList.Where(x => x.GroupKey == groupKey && x.KeyId != keyId && isEqualDateDisplayUnit(unit, x.ScheduleDate, keyDate, monthStartNendo)).ToList();
            }
        ...
        public class ScheduleListConverterNoRank : ScheduleListConverter
        {
            ...
            protected override List<Dao.ScheduleList.Get> getGroupDataList(IList<Dao.ScheduleList.Get> dataList, string keyId, long? groupKey, ScheduleDisplayUnit unit, DateTime keyDate, int monthStartNendo)
            {
        ...
        public static int GetUserFactoryId(string pUserId, ComDB db)
        {
            ...
            return user.LocationStructureId;
        }
