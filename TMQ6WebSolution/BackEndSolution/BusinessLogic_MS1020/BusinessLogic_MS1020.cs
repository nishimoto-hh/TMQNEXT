...
using Dao = BusinessLogic_MS1020.BusinessLogicDataClass_MS1020;
using Master = CommonTMQUtil.CommonTMQUtil.ComMaster;
using TMQUtil = CommonTMQUtil.CommonTMQUtil;

namespace BusinessLogic_MS1020
{
    /// <summary>
    /// 原因性格マスタ
    /// </summary>
    public class BusinessLogic_MS1020 : CommonBusinessLogicBase
    {
        #region 定数
        /// <summary>
        /// 構成グループID
        /// </summary>
        private static int structureGroupId = 1020;
        ...
        {
            /// <summary>SQL名：原因性格アイテム一覧取得(標準アイテム)</summary>
            public const string GetFailureCausePersonalityStructureStandardItemList = "GetFailureCausePersonalityStructureStandardItemList";
            /// <summary>SQL名：原因性格アイテム一覧取得(工場アイテム)</summary>
            public const string GetFailureCausePersonalityStructureFactoryItemList = "GetFailureCausePersonalityStructureFactoryItemList";
            /// <summary>SQL名：原因性格アイテム情報取得</summary>
            public const string GetFailureCausePersonalityStructureItemInfo = "GetLFailureCausePersonalityStructureItemInfo";
            /// <summary>SQL名：構成ID取得</summary>
            public const string GetStructureId = "GetStructureId";
            /// <summary>SQL名：原因性格アイテム表示順一覧取得</summary>
            public const string GetFailureCausePersonalityStructureIItemOrderList = "GetFailureCausePersonalityStructureItemOrderList";
            /// <summary>SQL名：構成マスタ登録</summary>
            public const string InsertFailureCausePersonalityStructureInfo = "InsertFailureCausePersonalityStructureInfo";
            ///// <summary>SQL名：構成マスタ更新</summary>
            //public const string UpdateMsStructureInfo = "UpdateMsStructureInfo";
            /// <summary>SQL名：原因性格翻訳マスタ削除</summary>
            public const string DeleteFailureCausePersonalityStructureOrder = "DeleteFailureCausePersonalityStructureOrder";
            /// <summary>SQL名：翻訳マスタ件数取得</summary>
            public const string GetCountLayersTranslation = "GetCountLayersTranslation";

            /// <summary>SQL格納先サブディレクトリ名</summary>
            public const string SubDir = Master.SqlName.SubDir + @"\FailureCausePersonalityStructure";
        ...
        /// コンストラクタ
        /// </summary>
        public BusinessLogic_MS1020() : base()
        {
        }
        #endregion
        ...
        private bool searchList()
        {
            ...
            if (hiddenInfo.ItemListType != (int)Master.ItemListType.Factory)
            {
                ...
                // 一覧のコントロールID
                var ctrlId = Master.ConductInfo.FormList.ControlId.StarndardItemId;
                // ページ情報取得
                var pageInfo = GetPageInfo(ctrlId, this.pageInfoList);
                ...
                TMQUtil.GetFixedSqlStatement(SqlName.SubDir, SqlName.GetFailureCausePersonalityStructureStandardItemList, out string baseSql, listUnComment);
            ...
            if (hiddenInfo.ItemListType != (int)Master.ItemListType.Standard)
            {
                ...
                // SQL取得(上記で取得したNullでないプロパティ名をアンコメント)
                TMQUtil.GetFixedSqlStatement(SqlName.SubDir, SqlName.GetFailureCausePersonalityStructureFactoryItemList, out string baseSql, listUnComment);
        ...
        private bool searchItemInfo(TMQUtil.HiddenInfoForMaster condition)
        {
            ...
            // 検索実行
            var results = TMQUtil.SqlExecuteClass.SelectList<Dao.SearchResult>(SqlName.GetFailureCausePersonalityStructureItemInfo, SqlName.SubDir, condition, this.db);
            if (results == null)
            {
                return false;
            }
        ...
        private bool searchItemOrderList(TMQUtil.HiddenInfoForMaster condition)
        {
            ...
            // 検索実行
            var results = TMQUtil.SqlExecuteClass.SelectList<TMQUtil.SearchResultForMaster>(SqlName.GetFailureCausePersonalityStructureIItemOrderList, SqlName.SubDir, condition, this.db);
        ...
        private bool executeRegistOrder()
        {
            ...
            //}
            // 工場別アイテム表示順マスタ削除
            if (!TMQUtil.DeleteDb(hiddenInfo, SqlName.DeleteFailureCausePersonalityStructureOrder, this.db, SqlName.SubDir))
            {
                return false;
            }
        ...
        private bool isErrorExclusive(TMQUtil.HiddenInfoForMaster hiddenInfo)
        {
            ...
            if (hiddenInfo.TargetItemList == (int)Master.TargetItemList.Standard && hiddenInfo.FactoryId != Const.CommonFactoryId)
            {
                ...
                // 排他チェック
                if (isErrorExclusiveByUpdateTime(condition, ConductInfo.FormEdit.ControlId.ItemInfoId))
                {
                    return true;
                }
            }
        ...
        private bool isErrorExclusiveByUpdateTime(TMQUtil.HiddenInfoForMaster condition, string ctrlId)
        {
            ...
            if (ctrlId == Master.ConductInfo.FormOrder.ControlId.ItemOrderId)
            {
                ...
                // 検索実行
                newList = TMQUtil.SqlExecuteClass.SelectList<TMQUtil.SearchResultForMaster>(SqlName.GetFailureCausePersonalityStructureIItemOrderList, SqlName.SubDir, condition, this.db);
            }
            else
            {
                // 未使用フラグ更新の場合
                ...
                // 検索実行
                newList = TMQUtil.SqlExecuteClass.SelectList<TMQUtil.SearchResultForMaster>(SqlName.GetFailureCausePersonalityStructureItemInfo, SqlName.SubDir, condition, this.db);
            }
        ...
        private bool isErrorRegist(TMQUtil.HiddenInfoForMaster hiddenInfo)
        {
            ...
            bool isErrorRegistForItemTranList(TMQUtil.HiddenInfoForMaster hiddenInfo, ref List<Dictionary<string, object>> errorInfoDictionary)
            {
                ...
                foreach (var rowDic in targetDicList)
                {
                    ...
                    // 重複チェック
                    var errFlg = false;
                    if (!MS1020_CheckDuplicateByItemTran(hiddenInfo, result, ref errFlg))
                    {
                        return false;
                    }
                ...
                /// <returns>エラーの場合False</returns>
                bool MS1020_CheckDuplicateByItemTran(
                    ...
                    ref bool errFlg)
                {
                    ...
                    //if (hiddenInfo.TargetItemList == Master.TargetItemList.Standard)
                    //{
                    //    // 標準アイテム一覧

                    //    // 構成グループに同じ翻訳が存在するか検索するSQL
                    //    sqlName = Master.SqlName.GetCountAllTranslation;
                    //}
                    //else
                    //{
                    //    // 構成グループの標準アイテムと工場アイテムに同じ翻訳が存在するか検索するSQL
                    //    sqlName = Master.SqlName.GetCountTranslation;
                    //}
                    ...
                    // 件数取得
                    int cnt = 0;
                    if (!TMQUtil.GetCountDb(targetInfo, sqlName, ref cnt, db, Master.SqlName.SubDir))
                    {
                        return false;
                    }
