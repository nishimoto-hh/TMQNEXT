...
namespace BusinessLogic_MS1720
{
    /// <summary>
    /// 仕入先マスタ
    /// </summary>
    public class BusinessLogic_MS1720 : CommonBusinessLogicBase
    {
        #region 定数
        /// <summary>
        /// 構成グループID
        /// </summary>
        private static int structureGroupId = 1720;
        ...
        public static class ExcelPortMasterListInfo
        {
            ...
            /// 仕入先コード(拡張項目1)列番号
            /// </summary>
            public const int ExData1ColumnNo = 12;
        }
        #endregion
        ...
        /// コンストラクタ
        /// </summary>
        public BusinessLogic_MS1720() : base()
        {
        }
        #endregion
        ...
        private bool executeCheckAndRegistExcelPort(ref List<TMQUtil.CommonExcelPortMasterList> resultList, ref List<ComDao.UploadErrorInfo> errorInfoList)
        {
            ...
            bool isErrorExcelPort(TMQUtil.CommonExcelPortMasterList registInfo, ref List<ComDao.UploadErrorInfo> errorInfoList)
            {
                ...
                {
                    // 仕入先コードは既に登録されています。
                    errorInfoList.Add(TMQUtil.setTmpErrorInfo((int)registInfo.RowNo, ExcelPortMasterListInfo.ExData1ColumnNo, GetResMessage(new string[] { ComRes.ID.ID111120131 }), GetResMessage(new string[] { ComRes.ID.ID941260001, ComRes.ID.ID111120131 }), TMQUtil.ComReport.LongitudinalDirection, registInfo.ProcessId.ToString()));
                    return true;
                }
        ...
        private bool executeExcelPortRegist(TMQUtil.CommonExcelPortMasterList result, DateTime now)
        {
            ...
            switch (result.ProcessId)
            {
                ...
                case TMQConst.SendProcessId.Update: // 更新
                    ...
                    // アイテムマスタ拡張登録
                    if (!TMQUtil.registItemExExcelPort(result, itemId, 1, now, this.db, this.UserId))
                    {
                        return false;
                    }
            ...
            foreach (var rowDic in targetDicList)
            {
                ...
                if (string.IsNullOrEmpty(itemInfo.ExData1) == false)
                {
                    if (!TMQUtil.CheckDuplicateByExItem1Tran(hiddenInfo, itemInfo, 1, true, this.db))
                    {
                        ...
                        // エラー
                        isErrorRow = true;
                        string errMsg = GetResMessage(new string[] { ComRes.ID.ID941260001, "111120131" });     // 仕入先コードは既に登録されています。
