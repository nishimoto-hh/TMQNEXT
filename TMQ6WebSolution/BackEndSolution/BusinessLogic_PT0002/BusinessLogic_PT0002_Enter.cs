...
namespace BusinessLogic_PT0002
{
    ...
    public partial class BusinessLogic_PT0002 : CommonBusinessLogicBase
    {
        ...
        private bool outputLabelEnter()
        {
            // ラベル出力クラス
            TMQUtil.Label label = new(this.db, this.LanguageId);

            // 入庫一覧で選択されたレコードを取得
            var selectedList = getSelectedRowsByList(this.resultInfoDictionary, TargetCtrlId.EnterResults);
            ...
            foreach (Dictionary<string, object> selectedRow in selectedList)
            {
                ...
                // ディクショナリをデータクラスに変換
                SetDataClassFromDictionary(selectedRow, TargetCtrlId.EnterResults, condition);
