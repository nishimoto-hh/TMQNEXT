...
SELECT mcp.management_standards_component_id,        -- 機器別管理基準部位ID
    ...
    ,CONCAT_WS('|',mcp.machine_id, ie.extension_data ) AS same_mark_key
...
FROM mc_management_standards_component mcp -- 機器別管理基準部位
    ...
    ON  (
            msc.maintainance_kind_structure_id = st.structure_id
...
ORDER BY maintainance_kind_structure_id,inspection_site_structure_id,management_standards_component_id
,inspection_content_structure_id,management_standards_content_id
