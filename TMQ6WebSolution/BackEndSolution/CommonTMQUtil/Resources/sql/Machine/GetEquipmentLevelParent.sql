WITH wi_machine_name AS( --「機器名称」取得用
    SELECT
        machine.machine_id,
...
wi_ma AS( --「機器」取得
    ...
    FROM
        mc_machine_parent_info child
        INNER JOIN
            wi_pa
        ON  child.parent_moto_id = wi_pa.parent_id
        LEFT JOIN
            wi_machine_name
        ON  child.machine_id = wi_machine_name.machine_id
)
/*****************************    自分自身が「親機」    ****************************/
SELECT
    CASE
