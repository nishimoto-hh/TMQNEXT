...
FROM
    ...
    LEFT OUTER JOIN ms_translation tr                               -- 翻訳
        ON it.control_id = tr.translation_id                        -- コントロールID = 翻訳ID
WHERE
...
AND 
    tr.location_structure_id = @FactoryId                           -- 工場ID
AND 
    tr.language_id = @LanguageId                                    -- 言語ID
