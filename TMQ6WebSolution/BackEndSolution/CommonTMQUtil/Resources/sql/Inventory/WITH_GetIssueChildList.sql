...
, translate_target AS(
         /*******************受払履歴テーブルより抽出*******************/
    SELECT DISTINCT
        FORMAT(plt.receiving_datetime, 'yyyy/MM/dd') AS inout_datetime --入庫日(受払日時)
        , plt.lot_no AS inout_no                    --入庫No(ロットNo)
        , pls.parts_location_id                     --棚ID
        , pls.parts_location_detail_no              --棚枝番
        , plt.unit_price                            --入庫単価
        , plt.currency_structure_id                 --金額管理単位ID
        , pih.inout_quantity                        --受払数(出庫数)
        , plt.unit_structure_id                     --数量管理単位ID
        , pih.inout_quantity * plt.unit_price AS issue_amount --受払数*入庫単価
    ...
    FROM
        ...
        LEFT JOIN inout_division                    --受払区分
            ON pih.inout_division_structure_id = inout_division.structure_id 
        LEFT JOIN department                        --部門
            ON plt.department_structure_id = department.structure_id 
        LEFT JOIN account                           --勘定科目
            ON plt.account_structure_id = account.structure_id 
    WHERE
    ...
    SELECT DISTINCT
        ...
        , pid.unit_price                                           --入庫単価
        , plt.currency_structure_id                                --金額管理単位ID
        , pid.inout_quantity                                       --受払数(出庫数)
        , plt.unit_structure_id                                    --数量管理単位ID
        , pid.inout_quantity * pid.unit_price AS issue_amount      --受払数*入庫単価
        , department.extension_data AS department_cd               --部門コード
        , plt.department_structure_id                              --部門ID
        , account.extension_data AS subject_cd                     --勘定科目コード
        , plt.account_structure_id                                 --勘定科目ID
