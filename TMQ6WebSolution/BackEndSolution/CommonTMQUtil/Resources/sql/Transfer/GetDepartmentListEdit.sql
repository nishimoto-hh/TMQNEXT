    ...
    (
      ...
      AND tra.location_structure_id = (
              ...
              FROM
                  structure_factory AS st_f
              WHERE
                  st_f.structure_id = history.account_structure_id
              AND st_f.factory_id IN(0, @UserFactoryId)
           )
      AND tra.structure_id = history.account_structure_id
    ) AS subject_nm,                                                                                                                                               -- 勘定科目
     (
          SELECT
          tra.translation_text
...
FROM
    ...
    LEFT JOIN
        department -- 部門
    ON  history.department_structure_id = department.department_id
    LEFT JOIN
        subject -- 勘定科目
    ON  history.account_structure_id = subject.subject_id
    LEFT JOIN
    ...
    LEFT JOIN -- 部門名称
        department_trans
    ON  history.department_structure_id = department_trans.department_id
WHERE
