SELECT
    item.structure_item_id AS warehouse_item_id,                        -- アイテムID
    item.structure_id AS warehouse_id,                                  -- 倉庫ID(構成ID)
    item.parent_structure_id AS warehouse_parent_id,                    -- 倉庫の親構成ID
    item.item_translation_id AS warehouse_item_translation_id,          -- 翻訳ID
    ROW_NUMBER() OVER(ORDER BY item.structure_id ASC) warehouse_number, -- 倉庫番号
        (
            SELECT
        ...
        (
            ...
            AND tra.structure_id = item.structure_id
        ) AS warehouse_name_before                                      -- 倉庫名(変更前)
FROM
