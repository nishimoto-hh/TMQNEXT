SELECT
    item.structure_item_id AS large_class_item_id,                        -- アイテムID
    item.structure_id AS large_class_id,                                  -- 機種大分類ID(構成ID)
    item.parent_structure_id AS large_class_parent_id,                    -- 機種大分類の親構成ID
    item.item_translation_id AS large_class_item_translation_id,          -- 翻訳ID
    ROW_NUMBER() OVER(ORDER BY item.structure_id ASC) large_class_number, -- 機種大分類番
        (
            SELECT
        ...
        (
            ...
            AND tra.structure_id = item.structure_id
        ) AS large_class_name_before                                      -- 機種大分類名(変更前)
FROM
