...
WHERE
    hi.summary_id = @SummaryId                              -- 自身と同一の保全活動件名
AND mc.machine_id = @MachineId                              -- 自身と同一の機器
AND hic.history_inspection_content_id NOT IN @ContentIdList -- バックエンド側に渡ってこなかった内容ID
AND COALESCE(hm.used_days_machine, -1) != @UsedDaysMachine  -- 機器使用期間が自身の値と異なる