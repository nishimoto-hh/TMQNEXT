SELECT
    COUNT(hic.history_inspection_content_id) 
FROM
...
WHERE
    hm.history_machine_id = @HistoryMachineId 
    AND ( 
        his.history_inspection_site_id != @HistoryInspectionSiteId 
        OR ( 
            his.history_inspection_site_id = @HistoryInspectionSiteId 
            AND hic.history_inspection_content_id != @HistoryInspectionContentId
        )
    )