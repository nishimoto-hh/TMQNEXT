...
, ma_summary_with_factory AS ( 
    --保全活動件名テーブルの場所階層IDから各レコードの工場IDを取得
    SELECT
        *
        , dbo.get_target_layer_id(location_structure_id, 1) AS factory_id --このレコードの工場ID
    FROM
        ma_summary 
    WHERE
        --件数を減らすため場所階層で絞り込みを行っておく(職種機種はmc_machine.job_structure_idを使用するためここでは絞り込みをしない)
        EXISTS ( 
            SELECT
                * 
            FROM
...
, summary_list AS ( 
    SELECT
        ...
        , CASE 
            ...
            ELSE hf.maintenance_site --故障情報 
            END AS maintenance_site_name -- 保全部位(翻訳)
        , CASE 
            WHEN su.activity_division = 1 
                THEN ( 
                        SELECT
            ...
            ELSE hf.maintenance_content --故障情報 
            END AS maintenance_content_name -- 保全内容(翻訳)
        ...
        , su.factory_id                         --このレコードの工場ID
    FROM
        ma_summary_with_factory su 
...
, target_data AS ( 
    -- 詳細検索条件で絞り込み後の保全活動件名IDを取得
    SELECT DISTINCT
        summary_id
    FROM
        ( --ADD_GetMaintenanceList.sqlの最初に終了の括弧有り
