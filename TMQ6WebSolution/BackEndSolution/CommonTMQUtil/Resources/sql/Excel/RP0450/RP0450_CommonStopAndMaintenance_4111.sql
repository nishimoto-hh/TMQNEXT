...
, main as (
    ...
    select
        5 as sort
        , (select maintenance_count from stop_summary where stop_division = '40') as data
    -- 機能停止の系停止時間
    union 
    select
        6 as sort
        , (select stop_time from stop_summary where stop_division = '40') as data
    -- 影響無しのカウント件数
    union 
    select
        7 as sort
        , (select maintenance_count from stop_summary where stop_division = '50') as data
    -- 影響無しの系停止時間
    union 
    select
        8 as sort
        , (select stop_time from stop_summary where stop_division = '50') as data
    -- 故障除外のカウント件数
    union 
    select
        9 as sort
        , (select maintenance_count from stop_summary where stop_division = '60') as data
    -- 故障除外の系停止時間
    union 
    select
        10 as sort
        , (select stop_time from stop_summary where stop_division = '60') as data
    -- 系停止区分の条件なしのカウント件数
    union 
    select
        11 as sort
