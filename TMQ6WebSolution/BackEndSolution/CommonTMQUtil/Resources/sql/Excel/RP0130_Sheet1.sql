SELECT
    ...
    , SUBSTRING(ma_request.request_content, 51,100) AS request_content_2                            -- 依頼内容２
    , SUBSTRING(ma_request.request_content,101,150) AS request_content_3                            -- 依頼内容３
    , SUBSTRING(ma_request.request_content,151,200) AS request_content_4                            -- 依頼内容４
    , SUBSTRING(ma_request.request_content,201,250) AS request_content_5                            -- 依頼内容５
    , SUBSTRING(ma_request.request_content,251,300) AS request_content_6                            -- 依頼内容６
    , SUBSTRING(ma_summary.plan_implementation_content,  1, 50) AS work_plan_1                      -- 作業計画１
    , SUBSTRING(ma_summary.plan_implementation_content, 51,100) AS work_plan_2                      -- 作業計画２
    , SUBSTRING(ma_summary.plan_implementation_content,101,150) AS work_plan_3                      -- 作業計画３
    , SUBSTRING(ma_summary.plan_implementation_content,151,200) AS work_plan_4                      -- 作業計画４
    , SUBSTRING(ma_summary.plan_implementation_content,201,250) AS work_plan_5                      -- 作業計画５
    , SUBSTRING(ma_summary.plan_implementation_content,251,300) AS work_plan_6                      -- 作業計画６
    ...
    , SUBSTRING(ma_history.maintenance_opinion, 41, 80) AS final_report_2                           -- 完了報告２
    , SUBSTRING(ma_history.maintenance_opinion, 81,120) AS final_report_3                           -- 完了報告３
FROM
    ma_request                                                          -- 保全依頼
