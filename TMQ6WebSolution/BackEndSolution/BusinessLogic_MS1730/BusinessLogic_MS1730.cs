    ...
    public class BusinessLogic_MS1730 : CommonBusinessLogicBase
    {
        ...
        private bool executeCheckAndRegistExcelPort(ref List<TMQUtil.CommonExcelPortMasterList> resultList, ref List<ComDao.UploadErrorInfo> errorInfoList)
        {
            ...
            bool isErrorExcelPort(TMQUtil.CommonExcelPortMasterList registInfo, ref List<ComDao.UploadErrorInfo> errorInfoList)
            {
                ...
                {
                    // 数量管理単位は既に登録されています。
                    errorInfoList.Add(TMQUtil.setTmpErrorInfo((int)registInfo.RowNo, ExcelPortMasterListInfo.ExData1ColumnNo, GetResMessage(new string[] { ComRes.ID.ID111130009 }), GetResMessage(new string[] { ComRes.ID.ID941260001, ComRes.ID.ID111130009 }), TMQUtil.ComReport.LongitudinalDirection, registInfo.ProcessId.ToString()));
                    return true;
                }
        ...
        private bool executeExcelPortRegist(TMQUtil.CommonExcelPortMasterList result, DateTime now)
        {
            ...
            switch (result.ProcessId)
            {
                ...
                case TMQConst.SendProcessId.Update: // 更新
                    ...
                    // アイテムマスタ拡張登録
                    if (!TMQUtil.registItemExExcelPort(result, itemId, 2, now, this.db, this.UserId))
                    {
                        return false;
                    }
