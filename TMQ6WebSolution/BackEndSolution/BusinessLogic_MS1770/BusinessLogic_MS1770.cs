...
namespace BusinessLogic_MS1770
{
    /// <summary>
    /// 勘定科目マスタ
    /// </summary>
    public class BusinessLogic_MS1770 : CommonBusinessLogicBase
    {
        #region 定数
        /// <summary>
        /// 構成グループID
        /// </summary>
        private static int structureGroupId = 1770;
        ...
        /// コンストラクタ
        /// </summary>
        public BusinessLogic_MS1770() : base()
        {
        }
        #endregion
        ...
        private bool executeCheckAndRegistExcelPort(ref List<TMQUtil.CommonExcelPortMasterList> resultList, ref List<ComDao.UploadErrorInfo> errorInfoList)
        {
            ...
            bool isErrorExcelPort(TMQUtil.CommonExcelPortMasterList registInfo, ref List<ComDao.UploadErrorInfo> errorInfoList)
            {
                ...
                {
                    // 勘定科目コードは既に登録されています。
                    errorInfoList.Add(TMQUtil.setTmpErrorInfo((int)registInfo.RowNo, ExcelPortMasterListInfo.ExData1ColumnNo, GetResMessage(new string[] { ComRes.ID.ID111060062 }), GetResMessage(new string[] { ComRes.ID.ID941260001, ComRes.ID.ID111060062 }), TMQUtil.ComReport.LongitudinalDirection, registInfo.ProcessId.ToString()));
                    return true;
                }
                ...
                if (string.IsNullOrEmpty(itemInfo.ExData1) == false)
                {
                    ...
                    else
                    {
                        if (!TMQUtil.CheckDuplicateByExItem1Tran(hiddenInfo, itemInfo, 1, false, this.db))
                        {
                            ...
                            // エラー
                            isErrorRow = true;
                            string errMsg = GetResMessage(new string[] { ComRes.ID.ID941260001, "111060062" });     // 勘定科目コードは既に登録されています。
                            string val = info.getValName("ex_data1");
                            errorInfo.setError(errMsg, val);
                            isError = true;
                        }
