...
namespace BusinessLogic_LN0002
{
    ...
    public partial class BusinessLogic_LN0002 : CommonBusinessLogicBase
    {
        ...
        private void initBudgetOutput()
        {
            ...
            //******************************************************************
            // 一覧画面で選択された行の内容を取得
            var selectedParams = getSelectedRowsByList(this.searchConditionDictionary, ConductInfo.FormList.CtrlId.List);
            // データクラスに変換
            //var paramClassList = convertDicListToClassList<ComDao.LnLongPlanEntity>(selectedParams, ConductInfo.FormList.CtrlId.List, new List<string> { "LongPlanId" });
            var paramClassList = convertDicListToClassList<ComDao.McMachineEntity>(selectedParams, ConductInfo.FormList.CtrlId.List, new List<string> { "MachineId" });
            ...
            paramClassList = paramClassList.GroupBy(x => x.MachineId).Select(x => x.First()).ToList();
            // ページ情報取得
            var pageInfo = GetPageInfo(ConductInfo.FormBudgetOutput.ControlId.MachineId, this.pageInfoList);
            // 画面にセット
            SetSearchResultsByDataClass<ComDao.McMachineEntity>(pageInfo, paramClassList, paramClassList.Count);

            //********************************************************************
            //*** 一覧画面のスケジュール表示条件を予算出力画面の隠し項目に設定 ***
            //********************************************************************
            // 一覧画面のスケジュール表示条件を取得
            var scheduleCond = GetFormDataByCtrlId<Dao.BudgetOutputScheduleCondition>(ConductInfo.FormList.CtrlId.ScheduleCondition, false);
