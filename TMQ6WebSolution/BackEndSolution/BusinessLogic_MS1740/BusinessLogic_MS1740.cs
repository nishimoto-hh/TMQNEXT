...
namespace BusinessLogic_MS1740
{
    /// <summary>
    /// 金額管理単位マスタ
    /// </summary>
    public class BusinessLogic_MS1740 : CommonBusinessLogicBase
    {
        #region 定数
        /// <summary>
        /// 構成グループID
        /// </summary>
        private static int structureGroupId = 1740;
        ...
        /// コンストラクタ
        /// </summary>
        public BusinessLogic_MS1740() : base()
        {
        }
        #endregion
        ...
        protected override int ExcelPortDownloadImpl(ref string fileType, ref string fileName, ref MemoryStream ms, ref string resultMsg, ref string detailMsg)
        {
            ...
            whereParam.StructureGroupId = structureGroupId; // 構成グループID(1740)
        ...
        private bool isErrorRegistForItemEx1List(TMQUtil.HiddenInfoForMaster hiddenInfo, ref List<Dictionary<string, object>> errorInfoDictionary)
        {
            ...
            foreach (var rowDic in targetDicList)
            {
                ...
                if (!TMQUtil.CheckDuplicateByExItem1Tran(hiddenInfo, itemInfo, 1, false, this.db))
                {
                    ...
                    // エラー
                    isErrorRow = true;
                    string errMsg = GetResMessage(new string[] { ComRes.ID.ID941260001, "111070042" });     // 金額管理単位は既に登録されています。
