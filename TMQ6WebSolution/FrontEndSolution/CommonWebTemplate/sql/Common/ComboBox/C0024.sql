/*
 * 数量管理単位・金額管理単位を取得(拡張に小数点以下桁数を取得)
 */
-- C0024
...
SELECT
    ...
    COALESCE(item_ex.extension_data, '0') AS exparam1,
...
FROM
    ...
    ON  (
            item.structure_item_id = item_ex.item_id
        AND item_ex.sequence_no = 2
        )
    /*IF factoryIdList != null && factoryIdList.Count > 0*/
    -- 工場ごとに工場別表示順を取得する
...
WHERE
    item.structure_group_id = /*param1*/0
