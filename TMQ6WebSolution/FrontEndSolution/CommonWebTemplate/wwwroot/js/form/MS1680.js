/* ========================================================================
 *  機能名　    ：   【MS1680】文書種類_保全活動_故障分析(個別工場)_略図添付マスタ
 * ======================================================================== */
...
function getPath() {
    ...
    while (i--) {
        var match = scripts[i].src.match(/(^|.*\/)MS1680\.js$/);
        if (match) {
            root = match[1];
            break;
        }
...
function clickComConductSelectBtn(appPath, cmConductId, cmArticle, btn, fromCtrlId) {
    // アイテム検索画面
    SI0001_clickComConductSelectBtn(appPath, cmConductId, cmArticle, btn, fromCtrlId, ConductId_MS1680);
}
