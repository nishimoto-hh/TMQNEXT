...
function getPath() {
    ...
    while (i--) {
        var match = scripts[i].src.match(/(^|.*\/)LN0001\.js$/);
        if (match) {
            root = match[1];
            break;
        }
...
const FormDetail = {
    ...
    , Condition: {
        ...
        // 計画内容コンボの選択値の拡張項目の値
        , PlanContentExt: 4
        // 計画内容コンボの選択値の拡張項目の値(コンボの値が変わってもここは変わらない。一覧が再検索されるタイミングで変わる)
        , PlanContentExtNotChange: 7
...
function prevInitFormData(appPath, formNo, btnCtrlId, conditionDataList, listDefines, pgmId) {
    ...
    if (formNo == FormSelect.No) {
        ...
        // 遷移先情報を設定(URL指定起動)
        conditionDataList = getParamByUrl(LN0001_ConductId, conditionDataList);
...
function prevTransForm(appPath, transPtn, transDiv, transTarget, dispPtn, formNo, ctrlId, btn_ctrlId, rowNo, element) {
    ...
    if (isScheduleLink(btn_ctrlId)) {
        ...
        // 保全項目一覧がどの計画内容で表示されているか取得
        var planExVal = getValue(FormDetail.Condition.Id, FormDetail.Condition.PlanContentExtNotChange, 0, CtrlFlag.Label);

        /* 以下の場合は確認メッセージを表示せずに保全活動に遷移する
         * ・計画内容が「保全項目」以外で表示されている
         * ・一覧画面のリンクがクリックされた場合(パラメータのctrlIdが一覧画面のIDかどうか)
         * ・クリックされたリンクが保全履歴が完了済み「●」
         * ・クリックされたリンクの上位ランクが履歴完了済み「▲」
         * ・クリックされたリンクが保全活動作成済み「◎」
         */
        if (planExVal != FormDetail.Condition.PlanComboOptValue.Maintainance ||
            ctrlId.indexOf(FormList.List.Id) > -1 ||
            conditionDataList[0][TransParamForMA0001ByLink.ScheduleCtrlNo] == ScheduleStatus.Complete ||
    ...
    if (formNo == FormDetail.No) {
        ...
        if (btn_ctrlId == FormDetail.Button.AddSubject) {
            //件名添付ボタン押下時
            // 共通処理
...
function postBuiltTabulator(tbl, id) {
    ...
    if (id == "#" + FormDetail.List.Id + getAddFormNo() || id == "#" + FormDetail.ListMaintKind.Id + getAddFormNo()) {
        ...
        // 計画内容コンボボックスで選択されている項目の拡張項目を非表示一覧に設定する
        var planExVal = getValue(FormDetail.Condition.Id, FormDetail.Condition.PlanContentExt, 0, CtrlFlag.Label);
        setValue(FormDetail.Condition.Id, FormDetail.Condition.PlanContentExtNotChange, 0, CtrlFlag.Label, planExVal, false, false);
...
function clickComConductSelectBtn(appPath, cmConductId, cmArticle, btn, fromCtrlId) {
    ...
    // 担当者検索画面
    SU0001_clickComConductSelectBtn(appPath, cmConductId, cmArticle, btn, fromCtrlId, ConductId_LN0001);
...
function checkTransitionForMA0001(appPath, formNo) {
    ...
    //Ajaxで件数を取得する
    ajaxCommon("checkExistsOtherScheduleLink", appPath, formNo, ConductId_LN0001, false, null, eventFunc);
