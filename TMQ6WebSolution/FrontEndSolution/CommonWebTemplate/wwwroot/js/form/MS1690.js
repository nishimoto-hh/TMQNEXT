/* ========================================================================
 *  機能名　    ：   【MS1690】文書種類_保全活動_故障分析(個別工場)_故障原因分析書添付マスタ
 * ======================================================================== */
...
function getPath() {
    ...
    while (i--) {
        var match = scripts[i].src.match(/(^|.*\/)MS1690\.js$/);
        if (match) {
            root = match[1];
            break;
        }
...
// 機能ID
const ConductId_MS1690 = "MS1690";

// 拡張項目列
const ItemExCol = { ExData1: 14, ExData2: 12, ExData3: 13 }
// 拡張項目コントロール列(TextBox: 0, Label: 1, Combo: 2, ChkBox: 3, Link: 4, Input: 5, Textarea: 6, Password: 7, Button: 8)
const ItemExCtrlCol = { ExData1: 0, ExData2: 0, ExData3: 0 }
...
function clickComConductSelectBtn(appPath, cmConductId, cmArticle, btn, fromCtrlId) {
    // アイテム検索画面
    SI0001_clickComConductSelectBtn(appPath, cmConductId, cmArticle, btn, fromCtrlId, ConductId_MS1690);
}
