/* ========================================================================
 *  機能名　    ：   【MS1660】文書種類_保全活動_故障分析_略図添付マスタ
 * ======================================================================== */
...
function getPath() {
    ...
    while (i--) {
        var match = scripts[i].src.match(/(^|.*\/)MS1660\.js$/);
        if (match) {
            root = match[1];
            break;
        }
...
function clickComConductSelectBtn(appPath, cmConductId, cmArticle, btn, fromCtrlId) {
    // アイテム検索画面
    SI0001_clickComConductSelectBtn(appPath, cmConductId, cmArticle, btn, fromCtrlId, ConductId_MS1660);
}
