...
function getPath() {
    ...
    while (i--) {
        var match = scripts[i].src.match(/(^|.*\/)MS1830\.js$/);
        if (match) {
            root = match[1];
            break;
        }
...
function prevTransForm(appPath, transPtn, transDiv, transTarget, dispPtn, formNo, ctrlId, btn_ctrlId, rowNo, element) {
    ...
    //    conditionData['STRUCTUREGROUPID'] = SearchItemStructureGroupID.Budget;	   // 構成グループID(1830)
...
//    // アイテム検索画面
//    SI0001_clickComConductSelectBtn(appPath, cmConductId, cmArticle, btn, fromCtrlId, ConductId_MS1830);
//}
//=================================================================
