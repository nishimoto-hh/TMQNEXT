...
function getPath() {
    ...
    while (i--) {
        var match = scripts[i].src.match(/(^|.*\/)MS2010\.js$/);
        if (match) {
            root = match[1];
            break;
        }
...
function clickComConductSelectBtn(appPath, cmConductId, cmArticle, btn, fromCtrlId) {
    // アイテム検索画面
    SI0001_clickComConductSelectBtn(appPath, cmConductId, cmArticle, btn, fromCtrlId, ConductId_MS2010);
}
