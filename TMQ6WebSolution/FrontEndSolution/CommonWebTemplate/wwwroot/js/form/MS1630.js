/* ========================================================================
 *  機能名　    ：   【MS1630】文書種類_機器台帳_MP情報タブマスタ
 * ======================================================================== */
...
function getPath() {
    ...
    while (i--) {
        var match = scripts[i].src.match(/(^|.*\/)MS1630\.js$/);
        if (match) {
            root = match[1];
            break;
        }
...
function clickComConductSelectBtn(appPath, cmConductId, cmArticle, btn, fromCtrlId) {
    // アイテム検索画面
    SI0001_clickComConductSelectBtn(appPath, cmConductId, cmArticle, btn, fromCtrlId, ConductId_MS1630);
}
