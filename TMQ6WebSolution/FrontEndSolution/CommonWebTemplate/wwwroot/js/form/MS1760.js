...
function getPath() {
    ...
    while (i--) {
        var match = scripts[i].src.match(/(^|.*\/)MS1760\.js$/);
        if (match) {
            root = match[1];
            break;
        }
...
// 機能ID
const ConductId_MS1760 = "MS1760";

// 拡張項目列
const ItemExCol = { ExData1: 11, ExData2: 12 }
// 拡張項目コントロール列(TextBox: 0, Label: 1, Combo: 2, ChkBox: 3, Link: 4, Input: 5, Textarea: 6, Password: 7, Button: 8)
const ItemExCtrlCol = { ExData1: 0, ExData2: 3 }
...
function clickComConductSelectBtn(appPath, cmConductId, cmArticle, btn, fromCtrlId) {
    // アイテム検索画面
    SI0001_clickComConductSelectBtn(appPath, cmConductId, cmArticle, btn, fromCtrlId, ConductId_MS1760);
}
