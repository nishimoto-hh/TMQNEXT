...
namespace CommonAPUtil.APInventoryUtil
{
    ...
    /// 在庫更新：出荷指図画面用
    /// </summary>
    public class ShippingDetailInventory
    {
        /// <summary>
        /// ログ出力用インスタンス
        /// </summary>
            ...
            List<InvDao.InventoryParameter> paramListInOutParam, string paramLanguage)
        {
            ...
            //"出荷指図：在庫更新処理"
            returnMessage = ComST.GetPropertiesMessage(new string[] { ComID.MB10083, ComID.MB10087 }, paramLanguage);
            ...
            foreach (var paramInfo in paramListInOutParam)
            {
                ...
                if (paramInfo.ProcessDivision == InvConst.INVENTORY_PROCESS_DIVISION.ADD)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(inOutSourceNo))
                    {
                        ...
                        orderInfo.ResultOrderNo = paramInfo.OrderNo;
                        orderInfo.ResultOrderLineNo1 = paramInfo.OrderLineNo1;
                        orderInfo.ResultOrderLineNo2 = paramInfo.OrderLineNo2;
                        orderInfo.InoutSourceNo = inOutSourceNo;

                        // 受注情報：受注受払ソース(受払予定)　引当(更新)：受払数更新
                        retVal = InvInout.InOutSourceRegist(db, InvInout.INOUT_MAKE_FLG.UPDATE_QTY_ONLY_SUBTRACTION, orderInfo, strLanguage, ref returnMessage);
                        if (!retVal)
                        {
                            break;
                        }
                ...
                else if (paramInfo.ProcessDivision == InvConst.INVENTORY_PROCESS_DIVISION.CANCEL)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(inOutSourceNo))
                    {
                        ...
                        // 受注情報：引当取消：受払ソース（受払予定）更新 (受払数加算）
                        retVal = InvInout.InOutSourceRegist(db, InvInout.INOUT_MAKE_FLG.UPDATE_QTY_ONLY_ADDITION, orderInfo, strLanguage, ref returnMessage);
                        if (!retVal)
                        {
                            break;
                        }
            ...
            string paramLanguageId, ref string paramMessage)
        {
            ...
            // 受払区分(値チェック)
            if (paramInfo.InoutDivision != ComConst.INOUT_DIVISION.SHIPPING)
            {
                //@行目の受払区分が正しくありません。
