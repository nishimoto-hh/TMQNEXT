...
namespace CommonAPUtil.APInventoryUtil
{
    ...
    /// 在庫更新：購買仕入：納期照会
    /// </summary>
    public class PurchaseAnswerInventory
    {
            ...
            foreach (var paramInfo in paramListInOutParam)
            {
                ...
                if (paramInfo.ProcessDivision == InvConst.INVENTORY_PROCESS_DIVISION.ADD)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(inOutSourceNo))
                    {
                        ...
                        //受払ソース更新
                        retVal = InvInout.InOutSourceRegist(db, InvInout.INOUT_MAKE_FLG.ANSWER, paramInfo, strLanguage, ref returnMessage);
                        if (!retVal)
                        {
                            break;
                        }
            ...
            string paramLanguageId, ref string paramMessage)
        {
            ...
            //受払区分(値チェック)
            if (paramInfo.InoutDivision != ComConst.INOUT_DIVISION.PURCHASE_DELIVERY_DATE)
            {
                //@行目の受払区分が正しくありません。
