...
namespace CommonAPUtil.APInventoryUtil
{
    ...
    public class OrderInventory
    {
            ...
            List<InvDao.InventoryParameter> paramListInOutParam, string paramLanguage)
        {
            ...
            //"受注：在庫更新処理"
            returnMessage = ComST.GetPropertiesMessage(new string[] { ComID.MB10083, ComID.MB10084 }, paramLanguage);
            ...
            foreach (var paramInfo in paramListInOutParam)
            {
                ...
                if (paramInfo.ProcessDivision == InvConst.INVENTORY_PROCESS_DIVISION.ADD)
                {
                    ...
                    if (boolFirst)
                    {
                        ...
                        paramInfo.InoutSourceNo = string.Empty;
                        boolFirst = false;
                    }
            ...
            string paramLanguageId, ref string paramMessage)
        {
            ...
            // 受払区分(値チェック)
            if ((paramInfo.InoutDivision != ComConst.INOUT_DIVISION.ORDER_NORMAL)
                && (paramInfo.InoutDivision != ComConst.INOUT_DIVISION.ORDER))
            {
                //@行目の受払区分が正しくありません。
